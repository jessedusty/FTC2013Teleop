...
#pragma config(Motor,  mtr_S1_C1_1,     armPivotor,    tmotorTetrix, openLoop)
#pragma config(Motor,  mtr_S1_C1_2,     robotLifter,   tmotorTetrix, openLoop)
#pragma config(Motor,  mtr_S1_C2_1,     ringLifterLength, tmotorTetrix, openLoop)
#pragma config(Motor,  mtr_S1_C2_2,     ringLifterAngle, tmotorTetrix, openLoop, encoder)
...
float cArmLenght, cArmAngle, cArmBaseRot;
...
int yThreshhold = 15;
int rotorAcc = 10; // clicks to not worry about accuracy

int rotorSpeedCalc(float target, float current) {
	if (target == current) return 0;
	int rotorSlow = 35; // accuracy tuning for rotor
	...
	if (abs(target-current) < rotorSlow) returnval = rotorSlow;
...
int armAngleSpeedCalc(float target, float current) {
	if (target == current) return 0;
	int rotorSlow = 35; // accuracy tuning for rotor
	int rotorFast = 70; // max slewing speed
...
void driveArmLength(float joystickval) {
