...
// THIS IS THE MAIN DRIVE CODE - FOR USE IN TELEOP, THIS FILE HAS BEEN CREATED ON MAY 18th, 2012 - thank you if you have read to this header :)
...
// Note: There are acynmys like TMT(task main thread) that can be used for going to a particular section in the code just press control-F and search for them


// SMC - Starting main code (enter global varibles and other stuff above this line!!!)


//MSIIL - make sure that the values going to the motor arn't less than -100 or greater than 100
...
// JDRM - this code is for the driving joystick (the one that actually moves the robot's position)
...
//discover the values for use in the driving rutine (using above methods)
void joyval_correct() {
  if ((normalctl) & (rev == 1)) {
    joyval_joystick1st();
...
// ACCST - accesory controler contols
